...
from app.oss.sts_token_error import StsTokenError
...
class AwsOssClient:
    def __init__(
        ...
        credentials, v, client = self.initClient()
        self.v = v
    ...
    def multipart_upload(self, filePath, fileName):
        ...
        try:
            self.client.upload_file(
                filePath, Bucket=self.bucket, Key=f"fit_zip/{fileName}", Config=config
            )
